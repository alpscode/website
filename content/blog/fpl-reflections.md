...
Like any sane person, I spent most of my time after that reading through rules, reviewing academic papers, taking notes from every guide I came across on Reddit. 
...
For this discussion, it's important to explain what I mean by "model" since there are a several different types. 
The models I mentioned above are called "predictive models", since they try to predict future performance. 
There are also another class of models called "prescriptive models" like optimization models to generate decisions. 

An optimization model consumes any kind of predictive model in order to produce a clear set of decisions. 
This is what's mostly missing in FPL circles: optimization models. 
While there are certainly many bots attempting to play well in FPL, I never real encountered anyone using else discussing optimization when I started playing aside from seen the occasional post about someone using it for a week or two. 
...
Adding premature chip policy logic forced me to use my WC (wildcard) chip by GW6. 
I received only 31 points in the GW that I used my first WC in... 
That was the GW I started to take things a bit more seriously as my competitiven side had started to kick in. 

## Challenging the *status quo* 

Around this time, I started mentioning on Twitter how I was using optimization for FPL purposes. 
The first knee-jerk reaction you often get from people after mentioning the use optimization models for FPL is "okay, but how did it perform?". 
This reaction is due in part the exaggerated expectation that these models should be perfect, similar to the AlphaZero program playing chess. 
Measuring a model’s performance using realized points is the wrong way to look at things though. 
FPL is a noisy problem with huge variance so when asking how an algorithm has performed after only a few weeks (or even after a single season) will result in a poor answer. 
...
5. I learned more about the finer details of FPL, like dealing with DGWs (double gameweeks, where at least one team plays 2 games), the importance of carrying a positive budget ITB (in the bank) and rolling free transfers, & the impact of ownership ratios. 
...
As if it was not enough that football itself is a low-scoring game with lots of variance, erratic changes (like the MUN-LIV game getting cancelled and COVID-19 postponements) which caused many weird schedule changes that I was not prepared for. 
...
It wasn't my original purpose, but I enjoyed my transformation from a casual quantitative player to a someone who applies optimization, writes about it, and creates tutorials for others to follow and learn from. 
...
At the end, I saw what I was thinking: even following an oversimplified setting, you can score great points (2482 was the maximum among entries) by only using optimization. 
...
I cannot wait until the new season starts (on August 13) to collect more data in order to improve my analytical models, beat Bas (and others), and claim the SAS mini-league title (as well as win the goody bag)! 
